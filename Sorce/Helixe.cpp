...
namespace curv
{
	...
	Helixe::Helixe(double *sp, double _r, double _h)
	{
		SetR(_r);
		SetH(_h);
		SetSpace(sp);
	...
	void  Helixe::SetSpace(double *sp)
	{
	...
	double *Helixe::GetPoint(double t)
	{
		auto p = new double[3];
		GetPoint(t, p);
		return p;
